...
export default {
  ...
  transformDOM: ({
    ...
    if (document.querySelector('.main-content')) {
      main.appendChild(document.querySelector('.main-content').cloneNode(true));
    }

    const banner = document.querySelector('.banner');
    if (banner) {
      const imageUrl = banner.getAttribute('data-msrc');
      const h1 = banner.querySelector('h1');
      const p = banner.querySelector('p');
      const img = document.createElement('img');
      img.src = imageUrl;
      main.appendChild(img);
      if (h1) main.appendChild(h1.cloneNode(true));
      const em = document.createElement('em');
      em.appendChild(p.cloneNode(true));
      main.appendChild(em);
      if (banner.nextElementSibling && banner.nextElementSibling.className === 'container') {
        main.append(banner.nextElementSibling);
      }
    }
